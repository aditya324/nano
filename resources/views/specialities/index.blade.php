...
@section('title', 'Medical Specialities in Bangalore | Nano Hospitals')
@section('meta_description', 'Explore all medical specialities at Nano Hospitals Bangalore — cardiology, orthopaedics, neurology, maternity, cancer care, and more across BG Road and Uttarahalli.')

@section('canonical', request('page') > 1 ? url()->current() . '?page=' . request('page') : url()->current())
@section('twitter_title', 'All Specialities | Nano Hospitals')
@section('twitter_description', 'All Specialities at Nano Hospitals. We provide the latest news and insights about healthcare, medical treatments, and hospital services.')
@section('content')
...
<div id="specialityGrid" class="flex flex-wrap justify-center gap-10 gap-y-10 mt-6 md:px-28 mb-10"></div>
...
<script>
    ...
    grid.innerHTML = specialities.map(item => {
        ...
        return `
<a href="${item.url}" aria-label="${item.title}" class="block">
